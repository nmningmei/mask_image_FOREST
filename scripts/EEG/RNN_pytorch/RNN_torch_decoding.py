            ...
            for fold,(idx_,idx_test) in enumerate(cv.split(X,targets)):
                ...
                for idx_epoch in range(n_epochs):
                    ...
                    for jj,(features,labels) in enumerate(dataloader_train):
                        ...
                        loss_batch += 0.001 * torch.norm(preds,1) + 1e-12
                        loss_batch.backward()
                        optimizer.step()
...
for a in classifier.parameters():
    a.requiers_grad = False
classifier.cpu()

classifier.zero_grad()
torch.manual_seed(12345)
np.random.seed(12345)
inputs = torch.rand(features.shape,).detach().requires_grad_(True)
temp = np.random.choice([0,1],size = batch_size)
temp = np.vstack([temp,1-temp]).T
targets = torch.from_numpy(temp).type(torch.FloatTensor)


opt = optim.Adam([inputs],lr = 1e-3)
...
for ii in range(50):
    ...
    print(loss.data)
    grad = torch.autograd.grad(loss,inputs,allow_unused=True)
    inputs.grad = grad[0]
    opt.step()

signal = inputs.permute(0,2,1).detach().numpy()
events = np.zeros((batch_size,3))
events[:,0] = np.arange(batch_size)
events[:,-1] = targets[:,-1]
event_id = {"living": 0,
            "nonliving":1}

info = mne.create_info(epochs.ch_names,100,ch_types = 'eeg',
                       montage = mne.channels.read_montage('standard_1020',ch_names=epochs.ch_names))

epoch_ = mne.EpochsArray(mne.decoding.Scaler(epochs.info).fit(epochs.get_data()).inverse_transform(signal),
                         info,events = events.astype(int),event_id = event_id)
epoch_['living'].average().plot_joint()

df = {f'time{ii}':[] for ii in range(timesteps)}
df['labels'] = []
for jj,(features,labels) in enumerate(dataloader_train):
    features = features.to(device).float()
    labels = labels.to(device).float()
    
    hidden = classifier.to(device).LSTM_layer(features)[0]
    
    for instance,ll in zip(hidden.detach().cpu().numpy(),
                           labels.detach().cpu().numpy()):
        for ii,time_point in enumerate(instance):
            df[f'time{ii}'].append(time_point[0])
        df['labels'].append(ll[-1])
df = pd.DataFrame(df)


x_means = df.groupby('labels').mean().reset_index().iloc[:,1:].values
x_sds = df.groupby('labels').std().reset_index().iloc[:,1:].values / np.sqrt(df.shape[0])

from matplotlib import pyplot as plt
fig,ax = plt.subplots()
for ii,(x_mean,x_sd) in enumerate(zip(x_means,x_sds)):
    ax.plot(epochs.times,x_mean,color = 'blue',label = f'{ii}')
...
df_plot = pd.DataFrame(xx_pca, columns = ['pc1','pc2'])
df_plot['labels'] = df['labels']
...
cv = StratifiedShuffleSplit(50,test_size = 0.1,random_state=12345)
clf = make_pipeline(PCA(),SVC(gamma='scale',random_state=12345))
cross_val_score(clf,xx,df['labels'].values,scoring = 'roc_auc',cv = cv).mean()

...
import os
import numpy as np
template = 'RNN_torch_decoding.py'
...
for ii,subject in enumerate(all_subjects):
    with open(f'RNN_torch_decoding_{ii+1}.py','w') as new_file:
        with open(template,'r') as old_file:
            for line in old_file:
                if "subject = '" in line:
                    line = f"    subject = '{subject}'\n"
                elif "mkl.set_num_threads" in line:
                    line = f"mkl.set_num_threads({cores})"
                new_file.write(line)
            old_file.close()
        new_file.close()
...
echo $(nproc)
pwd
python RNN_torch_decoding_{ii+1}.py
    """

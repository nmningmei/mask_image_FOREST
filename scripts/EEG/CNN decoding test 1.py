...
#                X_valid,_,_ = preprocess_features(X_valid,vectorizer,scaler)
                ...
                cnn         = cnn_block(inputs, filters = 64, idx_ = 1, beta = 1e-9)
                cnn         = cnn_block(cnn,    filters = 64, idx_ = 2, beta = 1e-8)
                cnn         = cnn_block(cnn,    filters = 64, idx_ = 3, beta = 1e-7)
                cnn         = cnn_block(cnn,    filters = 64, idx_ = 4, beta = 1e-6)
                cnn         = cnn_block(cnn,    filters = 64, idx_ = 5, beta = 1e-5)
                cnn         = cnn_block(cnn,    filters = 64, idx_ = 6, beta = 1e-4)
                rnn         = layers.GlobalAveragePooling1D(data_format='channels_last')(cnn)
...
#    print([np.mean(item[1]) for item in results])
                ...
                with open(os.path.join(report_dir,f'CNN_{subject}.txt'),'a') as f:

...
from utils import (compare_match,
                   extract_session_run_from_MRI,
                   read_behavorial_file,
                   extract)
import os
import csv
...
"""
df_temp             = [[int(extract_session_run_from_MRI(temp)[0]),
                        int(extract_session_run_from_MRI(temp)[1]),
                        temp] for temp in MRI_data]
...
df_temp['session']  = np.repeat(np.arange(1,19),3)
df_temp['block']    = np.tile(np.arange(1,4),18)
...
for ii,row in df_temp.iterrows():
    ...
    behavorial_file_picked  = behavorial_data[[compare_match(f,row['session'],row['block']) for f in behavorial_data]]
    n_day                   = row['day']
    n_block                 = row['block']
    n_session               = row['session']
    n_run                   = row['run']
    
    if n_day == 1:
        n_day = 7
    print()
    print(MRI_file,behavorial_file_picked[0],'day {}, block {}, session {}, run {}'.format(n_day,n_block,n_session,n_run))

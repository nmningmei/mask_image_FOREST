        ...
        if not os.path.join(output_dir,saving_name) in processed:
            ...
            for idx_train,idx_test in tqdm(zip(idxs_train,idxs_test)):
                reg         = linear_model.Ridge(normalize      = True,
                                                 random_state   = 12345)
#                reg         = linear_model.MultiTaskElasticNet(random_state = 12345,
#                                                               normalize = True,)
                
                reg         = GridSearchCV(reg,
                                           dict(alpha = np.logspace(-5,5,11),
#                                                l1_ratio = np.arange(0.,1.,0.2),
            ...
            print(f"{conscious_state},{encoding_model},VE = {np.nanmean(mean_variance):.4f},corr = {np.mean(corr):.4f},||weights|| = {np.mean([np.sum(np.abs(est.best_estimator_.coef_)) for est in regs]):.3}\n")

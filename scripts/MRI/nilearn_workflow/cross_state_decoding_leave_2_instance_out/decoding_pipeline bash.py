...
template = 'decoding_pipeline.py'
output_dir = '../decode_pipeline_bash'
...
for k in [1,2,3,4,5,6,7]:
    ...
    for conscious_state_source in ['unconscious','conscious']:
        for conscious_state_target in ['unconscious','conscious']:
            for key,folder_name in zip(['LOOST',
#                                        'LOOC',
#                                        'LOOBG',
                                        ],
                                       ['decoding_stratified_95',
#                                        'decoding_LOOC',
#                                        'decoding_loocv_balanced_glimpse',
                                        ]):
                if not np.logical_and(conscious_state_source == 'unconscious',
                    ...
                    with open(os.path.join(output_dir,created_file_name),'w') as new_file:
                        with open(template,'r') as old_file:
                            for line in old_file:
                                if "sub                     = 'sub-" in line:
                                    line = f"sub                     = '{sub}'\n"
                                elif "folder_name             = " in line:
                                    line = f"folder_name             = '{folder_name}'\n"
...
#PBS -N S{sub[-1]}_{conscious_state_source[0]}_{conscious_state_target[0]}_{key}
#PBS -o bash/out_sub{sub[-1]}_{conscious_state_source}_{conscious_state_target}_{key}.txt
#PBS -e bash/err_sub{sub[-1]}_{conscious_state_source}_{conscious_state_target}_{key}.txt
cd $PBS_O_WORKDIR
...
"""
                    print(content)
                    with open(f'{output_dir}/decode_{k}_{conscious_state_source}_{conscious_state_target}_{key}_q','w') as f:
                        f.write(content)
                        f.close()
                    with open(f'{output_dir}/qsub_jobs_decode_LOO.py','a') as f:
                        line = f'\nos.system("qsub decode_{k}_{conscious_state_source}_{conscious_state_target}_{key}_q")'
                        f.write(line)
                        f.close()

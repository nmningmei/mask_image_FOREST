    ...
    for conscious_state_source in ['unconscious','conscious']:
        for conscious_state_target in ['unconscious','conscious']:
            if not np.logical_and(conscious_state_source == 'unconscious',
                ...
                created_file_name = 'D_{}_{}_{}.py'.format(
...
"""
                print(content)
                with open(f'{output_dir}/decode_{k}_{conscious_state_source}_{conscious_state_target}_q','w') as f:
                    f.write(content)
                    f.close()
                with open(f'{output_dir}/qsub_jobs_decode_LOO.py','a') as f:
                    line = f'\nos.system("qsub decode_{k}_{conscious_state_source}_{conscious_state_target}_q")'
                    f.write(line)
                    f.close()

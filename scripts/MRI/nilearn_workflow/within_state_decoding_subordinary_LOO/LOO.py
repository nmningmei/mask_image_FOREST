...
def temp_roc_auc_score(y_true,y_test):
    try:
        return metrics.roc_auc_score(y_true,y_test,average = 'micro')
    except:
        return 0.5
scorer = metrics.make_scorer(temp_roc_auc_score,needs_proba = True)

sub                 = 'sub-01'
stacked_data_dir    = '../../../../data/BOLD_average/{}/'.format(sub)
output_dir          = '../../../../results/MRI/nilearn/{}/subordinary_LOO'.format(sub)
...
for conscious_state in ['unconscious','glimpse','conscious']:
    ...
    for model_name in model_names:
        ...
        if not os.path.exists(os.path.join(output_dir,file_name)):
            np.random.seed(12345)
            print(f'fitting {conscious_state}')
            ...
            with ignore_warnings(category = ConvergenceWarning):
                ...
                temp_score,permu_score,pval = permutation_test_score(pipeline,
                                                                     features,
                                                                     targets,
                                                                     cv = zip(idxs_train,idxs_test),
            ...
            results['chance_mean']          = permu_score.mean()
            results['chance_std']           = permu_score.std()
            
            print(f'{conscious_state}, {roi_name}, {model_name}, roc_auc = {np.mean(roc_auc):.4f}+/-{np.std(roc_auc):.4f}, p = {pval:.4f}')

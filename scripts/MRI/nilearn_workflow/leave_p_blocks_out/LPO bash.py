...
for ii,BOLD_data_file in enumerate(BOLD_data):
    for conscious_state in ['unconscious','glimpse','conscious']:
        with open(os.path.join(output_dir,f'{key_word}_{conscious_state}_{ii+1}.py'),'w') as new_file:
...
#PBS -N S{sub[-1]}_{conscious_state}_R{ii+1}
#PBS -o bash/out_{sub[-1]}_{conscious_state}_{ii+1}.txt
#PBS -e bash/err_{sub[-1]}_{conscious_state}_{ii+1}.txt
cd $PBS_O_WORKDIR
export PATH="/scratch/ningmei/anaconda3/bin:$PATH"
pwd
echo "{mask_name}"

python "{key_word}_{conscious_state}_{ii + 1}.py"
"""
        print(content)
        with open(f'{output_dir}/decode_{ii+1}_{conscious_state}_q','w') as f:
            f.write(content)
time = 1
content = '''
import os
...
with open(f'{output_dir}/qsub_jobs_decode_{key_word}.py','a') as f:
    for ii, BOLD_data_file in enumerate(BOLD_data):
        for conscious_state in ['unconscious','glimpse','conscious']:
            if counter == 0:
                f.write(f'\nos.system("qsub decode_{ii+1}_{conscious_state}_q")\n')
            else:

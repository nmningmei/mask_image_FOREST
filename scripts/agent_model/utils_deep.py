...
def performance_of_CNN_and_get_hidden_features(
    ...
    def behaviroal_measure(shuffle_ = True):
        ...
        if shuffle_:
            ...
            return np.array([roc_auc_score(y_true[idx],y_pred[idx]) for idx in idxs])
    behavioral = behaviroal_measure(False)
    
    gc.collect()
    beha_chance = Parallel(n_jobs = n_jobs,verbose = verbose)(delayed(behaviroal_measure)(**{
                               }) for i in range(n_permutation))
    beha_chance = np.array(beha_chance)
    
    mean_behavioral = behavioral.copy()
    mean_chance = np.array(beha_chance).copy()
    ps = ((np.sum(mean_chance.mean(1) > mean_behavioral.mean())) + 1) / (n_permutation + 1)
    if get_hidden and (hidden_model is not None):
        return behavioral,np.array(beha_chance).mean(1),ps,hidden_features,y_true,y_pred
    else:
        return behavioral,np.array(beha_chance).mean(1),ps

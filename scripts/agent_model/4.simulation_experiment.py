...
with tf.device('/physical_device:GPU:0'):
    ...
    simulation_saving_name = os.path.join(saving_dir,'scores as a function of decoder and noise.csv')
    
    # add noise to the images
    noise_levels = np.concatenate([a * 10 ** b for a in np.arange(1,10) for b in np.arange(4,8)]) #[1e2,1e3],
    ...
    for var in (noise_levels):
        if var not in df_temp['noise_level']:
            ...
            for decoder in ['linear-svm','rbf-svm','randomforest','knn']:
                res,permu_scores,ps_decode = utils_deep.decode_hidden(decoder,hidden_features,decode_targets,
                                                                      n_splits = n_splits,
